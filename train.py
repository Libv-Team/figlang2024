...
def train_model(image, prompt, label):
    prompt = "USER: <image>\n{}\nASSISTANT:\n".format(prompt)
...
def train(df):
    file_name = "gemini"
    result = checkpoint_utils.load_checkpoint("result/_{}.json".format(file_name))
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        def save():
            checkpoint_utils.save_checkpoint(result,
                                             "result/_{}.json".format(file_name))
    ...
    # 将DataFrame保存为CSV文件
    csv_filename = "result/{}.csv".format(file_name)
